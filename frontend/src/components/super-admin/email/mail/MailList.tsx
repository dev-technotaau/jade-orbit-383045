...
import type { MailListProps, MailFilter } from '@/components/super-admin/email/mail/props';
import type { MailSummary } from '@/types/email-mailbox';
...
/** Primary display name for a row: recipient in Sent/Drafts, otherwise sender. */
function primaryName(m: MailSummary, folderRole: string | null): string {
  const isSentLike = folderRole === 'sent' || folderRole === 'drafts';
  if (isSentLike) {
    const first = m.to[0];
    return first ? first.name || first.address : '(no recipient)';
  }
  return m.from ? m.from.name || m.from.address : '(unknown sender)';
}
...
export default function MailList({
  ...
  const allSelected = messages.length > 0 && messages.every((m) => selectedUids.includes(m.uid));
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="flex h-full flex-col items-center justify-center gap-2 p-8 text-center">
            ...
            <p className="text-sm text-[var(--text-secondary)]">No messages</p>
          </div>
        ) : (
          <ul>
            {messages.map((m) => {
              const isActive = activeUid === m.uid;
              ...
              return (
                <li key={m.uid}>
                  ...
                  >
                    ...
                    <Star
                      aria-hidden
                      fill={m.flagged ? AMBER : 'none'}
                      style={m.flagged ? { color: AMBER } : undefined}
                      className={cn(
                        'mt-0.5 h-4 w-4 shrink-0',
                        !m.flagged && 'text-[var(--text-tertiary)]',
                      )}
                    />
