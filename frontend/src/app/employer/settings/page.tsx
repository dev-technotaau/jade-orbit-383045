...
function AccountInfoSection() {
  ...
  const handleSaveType = () => {
    ...
    // Hiring type is a company-only question; individuals always hire
    // DIRECT (enforced again server-side, so stale clients can't desync).
    if (selectedAccountType === 'COMPANY' && !selectedHiringType) {
      showToast.error('Please select your hiring type');
      return;
    }
    saveMutation.mutate({
      accountType: selectedAccountType,
      hiringType: selectedAccountType === 'INDIVIDUAL' ? 'DIRECT' : selectedHiringType,
    });
  };
      ...
      <div className="space-y-4">
        ...
        {!editingType ? (
          <div className="space-y-3">
            ...
            {/* Companies only — individuals are auto-assigned DIRECT and
                never answer the hiring-type question, so don't surface it. */}
            {company?.accountType === 'COMPANY' && company?.hiringType && (
              <div>
                <label className="mb-1 block text-sm font-medium text-[var(--text)]">
                  Hiring Type
                </label>
          ...
          <div className="space-y-4 rounded-lg border border-[var(--border)] bg-[var(--bg-secondary)] p-4">
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {Object.entries(ACCOUNT_TYPE_LABELS).map(([value, label]) => (
                  <button
                    key={value}
                    type="button"
                    // Individuals hire DIRECT by definition — auto-set it
                    // and hide the hiring-type selector below.
                    onClick={() => {
                      setSelectedAccountType(value);
                      if (value === 'INDIVIDUAL') setSelectedHiringType('DIRECT');
                    }}
            ...
            {/* Hiring type — companies only (individuals auto-DIRECT). */}
...
function NotificationsTab() {
  ...
  /* Cadence / quiet hours share the notificationPreferences document with the
     channel toggles; held separately here, merged into one request on save. */
  ...
  useEffect(() => {
    ...
    if (p) {
      ...
      setPreferences((prev) => ({
        emailApplications: bool(p.emailApplications, prev.emailApplications),
        emailMessages: bool(p.emailMessages, prev.emailMessages),
        emailMarketing: bool(p.emailMarketing, prev.emailMarketing),
        smsAlerts: bool(p.smsAlerts, prev.smsAlerts),
        ...
        weeklyDigest: bool(p.weeklyDigest, prev.weeklyDigest),
      }));
    }
  }, [companyData]);
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      await employerService.updateCompany({
    ...
    <div className="space-y-6">
      <Card variant="bordered">
        ...
        <div className="mt-8 border-t border-[var(--border)] pt-8">
          <DigestPreferences role="employer" value={digestPrefs} onChange={setDigestPrefs} />
        </div>
